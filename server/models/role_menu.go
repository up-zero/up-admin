...
func GetRoleMenus(roleIdentity string, isAdmin bool) (*gorm.DB, error) {
	tx := new(gorm.DB)
	if isAdmin {
		tx = DB.Model(new(MenuBasic)).Select("id, parent_id, identity, name, web_icon, sort").Order("sort ASC")
	} else {
		...
		tx = DB.Model(new(RoleMenu)).Select("mb.id, mb.parent_id, mb.identity, mb.name, mb.web_icon, mb.sort").
