...
func Password(c *gin.Context) {
	...
	// 生成 token
	token, err := helper.GenerateToken(ub.ID, ub.Identity, ub.Username, define.TokenExpire)
	if err != nil {
		helper.Error("[BindJSON ERROR] : %v", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
	...
	// 生成 token
	refreshToken, err := helper.GenerateToken(ub.ID, ub.Identity, ub.Username, define.RefreshTokenExpire)
	if err != nil {
		helper.Error("[BindJSON ERROR] : %v", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
